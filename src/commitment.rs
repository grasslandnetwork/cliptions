...
mod tests {
    ...
    fn test_empty_salt() {
        ...
        assert!(matches!(
            result,
            Err(crate::error::CliptionsError::Commitment(
                CommitmentError::EmptySalt
            ))
        ));
    }
    ...
    fn test_batch_verification() {
        ...
        let commitments = vec![
            (
                "message1",
                "salt1",
                generator.generate("message1", "salt1").unwrap(),
            ),
            (
                "message2",
                "salt2",
                generator.generate("message2", "salt2").unwrap(),
            ),
