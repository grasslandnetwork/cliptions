...
fn benchmark_commitment_verification(c: &mut Criterion) {
    ...
    c.bench_function("commitment_verification", |b| {
        b.iter(|| verifier.verify(black_box(message), black_box(salt), black_box(&commitment)))
    });
}
...
fn benchmark_batch_commitment_verification(c: &mut Criterion) {
    ...
    for &size in &[10, 50, 100, 500] {
        ...
        group.bench_with_input(
            ...
            &commitment_refs,
            |b, refs| b.iter(|| verifier.verify_batch(black_box(refs))),
        );

        group.bench_with_input(
            BenchmarkId::new("parallel", size),
            &commitment_refs,
            |b, refs| b.iter(|| verifier.verify_batch_parallel(black_box(refs))),
        );
    }
...
fn benchmark_embedding_generation(c: &mut Criterion) {
    ...
    c.bench_function("text_embedding", |b| {
        b.iter(|| {
            embedder
                .get_text_embedding(black_box("test text for embedding"))
                .unwrap()
        })
    });
    ...
    c.bench_function("image_embedding", |b| {
        b.iter(|| {
            embedder
                .get_image_embedding(black_box("test_image.jpg"))
                .unwrap()
        })
    });
}
...
fn benchmark_scoring_strategies(c: &mut Criterion) {
    ...
    c.bench_function("clip_batch_scoring", |b| {
        b.iter(|| {
            strategy
                .calculate_score(black_box(&image_features), black_box(&text_features))
                .unwrap()
        })
    });
}
...
fn benchmark_payout_calculation(c: &mut Criterion) {
    ...
    for &num_participants in &[10, 25, 50, 100, 500] {
        ...
        group.bench_with_input(
            ...
            &ranked_results,
            |b, results| {
                b.iter(|| calculate_payouts(black_box(results), black_box(1000.0)).unwrap())
            },
        );
    }
        ...
        group.bench_with_input(
            ...
            |b, guesses| {
                b.iter(|| {
                    ...
                    let _payouts =
                        calculate_payouts(black_box(&rankings), black_box(1000.0)).unwrap();
                })
            },
        );
    }
