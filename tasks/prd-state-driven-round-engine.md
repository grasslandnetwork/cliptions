# PRD: State-Driven Block Engine

## 1. Introduction/Overview

This document outlines the requirements for a new state-driven engine to manage the lifecycle of Cliptions blocks. The core of this feature is to create a robust and autonomous application that understands the current state of a game block and intelligently determines the next logical action.
...
-   **Goal 1: Semi-Autonomous Operation:** Create an application that can orchestrate the steps of a block, prompting the validator for action and informing the miner of the current status.
...
-   **Goal 3: Establish a Clear State Machine:** Implement a well-defined block lifecycle using the Rust typestate pattern to ensure correctness at compile time.
...
-   **As a Validator,** I want the application to read the current block state from the database and prompt me to execute the next step, so that I can manage the game lifecycle efficiently and without errors.
-   **As a Miner,** I want the application to connect to Twitter, show me the live status of the active block based on the Validator's tweets, and help me craft my submission, so that I can participate in the game easily.

## 4. Functional Requirements

### FR1: Rust Typestate Pattern for Block Lifecycle
The application **must** implement the block lifecycle using the Rust typestate pattern. This ensures that operations can only be performed when the block is in the correct state, enforced by the compiler.

### FR2: Defined Block States
The block state machine will consist of the following distinct states:
1.  `Pending`
2.  `CommitmentsOpen`
3.  `CommitmentsClosed`
4.  `RevealsOpen`
...
-   The state of the block will be determined by parsing the content of tweets from the validator's account.
...
The application **must** support a fee-based system for block participation.
...
-   When started as a `validator`, the application will use the `cliptions-twitter-api` library to determine the current block state.
...
### FR7: Miner Workflow
-   When started as a `miner`, it will poll for validator tweets to understand the current block status.
...
### FR7: Data Schema
The block state and data **must** be encoded in the text of the validator's tweets. A clear, machine-parsable format for these tweets (e.g., using hashtags or key-value pairs like `STATE:CommitmentsOpen`) must be established to represent the block state, deadlines, and other relevant data.

## 5. Non-Goals (Out of Scope)

-   **Automatic Block Creation:** The validator must manually initiate the creation of a new block. The system will not automatically start a new block after one finishes.
...
-   **Typestate Pattern:** The API design should heavily lean on the typestate pattern. For example, a `Block<CommitmentsOpen>` struct would have a method like `close_commitments(self) -> Block<CommitmentsClosed>`, but not a method to open reveals. This enforces the lifecycle at the type level.
...
**Current State:**
- All existing Rust code is sync: `fn process_block_payouts() -> Result<Vec<ScoringResult>>`
...
- **Async State Transitions:** State machine methods must be async: `async fn close_commitments(self) -> Result<Block<CommitmentsClosed>>`. These methods will call the Twitter API library.
...
impl Block<CommitmentsOpen> {
    pub async fn close_commitments(self, client: &TwitterClient) -> Result<Block<CommitmentsClosed>> {
...
-   The application correctly fetches and parses the latest tweet from the validator's account to determine the current block's state.
-   A validator can successfully advance a block through its entire lifecycle by confirming the prompts, which results in posting a new status tweet.
-   A miner can successfully view the live state of a block by reading the validator's tweets and receives timely updates as the state changes.
-   When a block is complete, the validator instance waits for manual input to start a new block, while miner instances correctly enter a polling state, waiting for the new block.
